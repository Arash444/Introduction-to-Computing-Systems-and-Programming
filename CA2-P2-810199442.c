...
/*#define MESSAGES_FILE_ADDRESS "messages.txt"*/
...
int SpaceForMessage(char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int*, int*); /*no need to define pointers*/
void AddMessage(char input_for_message[MAX_MESSAGE_CHARACTERS + 2], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int*, int*); /*no need to define pointers*/
void ShowRecievedMessages(char username[NUMBER_OF_ACCOUNTS][MAX_USERNAME_CHARACTERS + 1], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int*); /*no need to define pointers*/
void ShowSentMessages(char username[NUMBER_OF_ACCOUNTS][MAX_USERNAME_CHARACTERS + 1], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int*); /*no need to define pointers*/
int PrintMessages(char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int*, int*); /*no need to define pointers*/
void GetOut(int*);
void Logout(int*);
void CreateFile(char phone_number[NUMBER_OF_ACCOUNTS][PHONE_NUMBER_NUMBER_DIGITS + 1], char username[NUMBER_OF_ACCOUNTS][MAX_USERNAME_CHARACTERS + 1], char password[NUMBER_OF_ACCOUNTS][MAX_PASSWORD_CHARACTERS + 1], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1]);
/*int CountNumberOfMessages(char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int, int);*/
int ReadFile(char phone_number[NUMBER_OF_ACCOUNTS][PHONE_NUMBER_NUMBER_DIGITS + 1], char username[NUMBER_OF_ACCOUNTS][MAX_USERNAME_CHARACTERS + 1], char password[NUMBER_OF_ACCOUNTS][MAX_PASSWORD_CHARACTERS + 1], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1]);
...
int main() /*Decription: Creates a new messenger with the options to signup, login and exit with the ability to send messages in the second menu*/
 {
	...
	{
		fclose(information);
		ReadFile(phone_number, username, password, message);
	}
	...
	CreateFile(phone_number, username, password, message);
}
int SpaceForAccount(char phone_number[NUMBER_OF_ACCOUNTS][PHONE_NUMBER_NUMBER_DIGITS + 1])
{
...
void CreateFile(char phone_number[NUMBER_OF_ACCOUNTS][PHONE_NUMBER_NUMBER_DIGITS + 1], char username[NUMBER_OF_ACCOUNTS][MAX_USERNAME_CHARACTERS + 1], char password[NUMBER_OF_ACCOUNTS][MAX_PASSWORD_CHARACTERS + 1], char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1])
{
	...
	/*	fopen_s(&messages, MESSAGES_FILE_ADDRESS, "w");
	for (int current_account = FIRST_ACCOUNT; current_account < NUMBER_OF_ACCOUNTS && *phone_number[current_account] != EMPTY; current_account++) /*Writes the user's information in a file.
	{
		for (int reciever_account = FIRST_ACCOUNT; reciever_account < NUMBER_OF_RECIEVERS; reciever_account++)
		{
			int count_message_numbers = CountNumberOfMessages(message, current_account, reciever_account);
			if (count_message_numbers == EMPTY)
			{
				continue;
			}
			else
			{
				fprintf(messages, "from: %d to: %d count: %d ", current_account, reciever_account, count_message_numbers);
			}
			for (int number_of_messages = FIRST_ELEMENT; number_of_messages < count_message_numbers; number_of_messages++)
			{
				fprintf(messages, "%s,", message[current_account][reciever_account][number_of_messages]);
			}
		}
		fprintf(messages, "\n");
	}
...
int CountNumberOfMessages(char message[NUMBER_OF_SENDERS][NUMBER_OF_RECIEVERS][MAX_NUMBER_OF_MESSAGES][MAX_MESSAGE_CHARACTERS + 1], int current_account, int reciever_account)
{
	int count_message_numbers = EMPTY;
	for (int i = FIRST_ELEMENT; i < MAX_MESSAGE_CHARACTERS; i++)
	{
		if (*message[current_account][reciever_account][i] != EMPTY)
		{
			count_message_numbers++;
		}
		else
		{
			return count_message_numbers;
		}
	}
	return count_message_numbers;*/
...
/*	if ((messages = fopen(MESSAGES_FILE_ADDRESS, "r")) == EMPTY)
	{
		return CORRECT;
	}
	fopen_s(&messages, MESSAGES_FILE_ADDRESS, "r");
	for (int i = FIRST_ELEMENT; i < NUMBER_OF_ACCOUNTS; i++) /*Writes the user's information in a file. We also calculate the length of the strings so the fwrite fucntion works properly. We'll also add a space so we can distinguish each of the values
	{
		fscanf_s(messages, "from: %d to: %d count: %d ", &current_account, &reciever_account, &count_message_numbers);
		for (int number_of_messages = FIRST_ELEMENT; number_of_messages < count_message_numbers; number_of_messages++)
		{
			for (int i = FIRST_ELEMENT; i < MAX_MESSAGE_CHARACTERS + 1; number_of_messages++)
			{
				if (fgetc(messages) == ',')
				{
					break;
				}
				message[current_account][reciever_account][number_of_messages][i] = fgetc(messages);
			}
		}
		fscanf_s(messages, "\n");
	}
